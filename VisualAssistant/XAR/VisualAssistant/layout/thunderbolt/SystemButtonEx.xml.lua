...
function System_Button_Ctrl_OnBind(self)
	...
	if attr.MiniShow then
		...
		miniobj:AttachListener("OnMouseHover", true, System_Item_OnMouseHover)
	...
	if attr.MaxShow then
		...
		maxobj:AttachListener("OnMouseHover", true, System_Item_OnMouseHover)
	...
	if attr.CloseShow then
		...
		closeobj:AttachListener("OnMouseHover", true, System_Item_OnMouseHover)
	...
	local lcLeft = 0
	--if not miniobj and not maxobj then
	--lcLeft = lcLeft + 10
	--end
	local left, top, right, bottom = self:GetObjPos()
	local SelfWidth = right - left
	local SelfHeight = bottom - top+5
	local ItemWidth = SelfWidth / lcIndex
	if miniobj then
	    if attr.ItemWidthIsSame then
			miniobj:SetObjPos(lcLeft, 0, ItemWidth+lcLeft, SelfHeight)
			lcLeft = lcLeft + ItemWidth
		else
		    miniobj:SetObjPos(lcLeft, 0, attr.MiniWidth+lcLeft, SelfHeight)
...
function System_Item_OnMouseHover(self)
--[[	local owner = self:GetOwnerControl()
	local attr = owner:GetAttribute()
	
	if attr.TipObj == nil then
		local objFactory = XLGetObject("Xunlei.UIEngine.ObjectFactory")
		local objFactory = XLGetObject("Xunlei.UIEngine.ObjectFactory")
		attr.TipObj = objFactory:CreateUIObject("System_Button.Tip", "Thunder.CommonTips")
	else
		return
	end
	...
	if attr.Mini_NormalImageID then
		if  objattr.BkgNormalImageID == attr.Mini_NormalImageID then  --最小化
			attr.TipObj:SetText("最小化")
		end
	end

	if attr.Max_NormalImageID then
		if  objattr.BkgNormalImageID == attr.Max_NormalImageID then  --最大化
			attr.TipObj:SetText("最大化")
		end
	end
	
	if attr.Restore_NormalImageID then
		if  objattr.BkgNormalImageID == attr.Restore_NormalImageID then  --还原
			attr.TipObj:SetText("还原")
		end
	end
	
	if attr.SingleClose_NormalImageID then
		if  objattr.BkgNormalImageID == attr.SingleClose_NormalImageID then  --关闭
			attr.TipObj:SetText("关闭")
		end
	end
	
	if attr.Close_NormalImageID then
		if  objattr.BkgNormalImageID == attr.Close_NormalImageID then  --只有关闭
			attr.TipObj:SetText("关闭")
		end
	end
	...
	attr.TipObj:SetType(4)
	owner:AddChild(attr.TipObj)
	local left,top,right,bottom = attr.TipObj:GetTipRect(self)
	local osShell = XLGetObject("Xunlei.UIEngine.OSShell")
	local screenx , screeny = osShell:GetScreenSize()
	local objTree = self:GetOwner()
	local hWnd = objTree:GetBindHostWnd()
	local absl, abst = self:GetAbsPos()
	local absl, abst = hWnd:ClientPtToScreenPt( absl, abst)	
	local finleft = l +width/2
	if right - left + absl + width/2 > screenx then 
		finleft = l  - (right - left + absl - screenx)
	end
	attr.TipObj:SetObjPos(finleft , height, right - left + finleft,bottom - top+height)
]]
end
...
function System_Item_OnMouseLeave(self, x, y)
	...
	if ownerattr.TipObj ~= nil then
		owner:RemoveChild(ownerattr.TipObj)
		ownerattr.TipObj = nil
	end
